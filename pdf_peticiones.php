...
if (!isset($_SESSION['nombre'])) {
    ...
    if (
        ...
        // Set width (6 columns)
        $pdf->SetWidths(array(16, 10, 30, 20, 20, 17, 15, 15, 19, 17, 17));
        ...
        $pdf->SetFont('Arial', '', 8);
        $pdf->SetX(-90);
        $pdf->Cell(10, 20, utf8_decode('ORD: '));
        $pdf->SetFont('Arial', 'B', 8);

        if ($ordinarioTamano == 1) {
            $pdf->SetX(-76);
            $pdf->cell(10, 20, $idNew->ordinario, 0, 0, 'R', 0);
            $pdf->Line(135, 22, 151, 22);
        } elseif ($ordinarioTamano == 2) {
            $pdf->SetX(-75);
            $pdf->cell(10, 20, $idNew->ordinario, 0, 0, 'R', 0);
            $pdf->Line(135, 22, 151, 22);
        } elseif ($ordinarioTamano == 3) {
            $pdf->SetX(-74);
            $pdf->cell(10, 20, $idNew->ordinario, 0, 0, 'R', 0);
            $pdf->Line(135, 22, 151, 22);
        } elseif ($ordinarioTamano == 4) {
            $pdf->SetX(-73);
            $pdf->cell(10, 20, $idNew->ordinario, 0, 0, 'R', 0);
            $pdf->Line(135, 22, 151, 22);
        }
        ...
        $pdf->SetY(22);
        $pdf->SetX(-102);
        $pdf->Cell(10, 10, 'REFERENCIA: ');
        $pdf->SetFont('Arial', 'B', 8);

        if ($referenciaTamano == 1) {
            $pdf->SetX(-76);
            $pdf->cell(10, 10, $idNew->referencia, 0, 0, 'R', 0);
            $pdf->Line(135, 29, 151, 29);
        } elseif ($referenciaTamano == 2) {
            $pdf->SetX(-75);
            $pdf->cell(10, 10, $idNew->referencia, 0, 0, 'R', 0);
            $pdf->Line(135, 29, 151, 29);
        } elseif ($referenciaTamano == 3) {
            $pdf->SetX(-74);
            $pdf->cell(10, 10, $idNew->referencia, 0, 0, 'R', 0);
            $pdf->Line(135, 29, 151, 29);
        } elseif ($referenciaTamano == 4) {
            $pdf->SetX(-73);
            $pdf->cell(10, 10, $idNew->referencia, 0, 0, 'R', 0);
            $pdf->Line(135, 29, 151, 29);
        } elseif ($referenciaTamano == 5) {
            $pdf->SetX(-72);
            $pdf->cell(10, 10, $idNew->referencia, 0, 0, 'R', 0);
            $pdf->Line(135, 29, 151, 29);
        } elseif ($referenciaTamano == 6) {
            $pdf->SetX(-71);
            $pdf->cell(10, 10, $idNew->referencia, 0, 0, 'R', 0);
            $pdf->Line(135, 29, 151, 29);
        } elseif ($referenciaTamano == 7 || $referenciaTamano == 8) {
            $pdf->SetX(-70);
            $pdf->cell(10, 10, $idNew->referencia, 0, 0, 'R', 0);
            $pdf->Line(135, 29, 151, 29);
        } elseif ($referenciaTamano == 9) {
            $pdf->SetX(-69);
            $pdf->cell(10, 10, $idNew->referencia, 0, 0, 'R', 0);
            $pdf->Line(135, 29, 151, 29);
        } elseif ($referenciaTamano = 10) {
            $pdf->SetX(-68);
            $pdf->cell(10, 10, $idNew->referencia, 0, 0, 'R', 0);
            $pdf->Line(135, 29, 151, 29);
        }
        ...
        $pdf->SetY(29);
        $pdf->SetX(-111);
        $pdf->Cell(10, 10, 'SOLIC DE COMPRA: ');
        $pdf->SetFont('Arial', 'B', 8);

        if ($solici_de_compraTamano == 1) {
            $pdf->SetX(-76);
            $pdf->cell(10, 10, $idNew->solici_de_compra, 0, 0, 'R', 0);
            $pdf->Line(135, 36, 151, 36);
        } elseif ($solici_de_compraTamano == 2) {
            $pdf->SetX(-75);
            $pdf->cell(10, 10, $idNew->solici_de_compra, 0, 0, 'R', 0);
            $pdf->Line(135, 36, 151, 36);
        } elseif ($solici_de_compraTamano == 3) {
            $pdf->SetX(-74);
            $pdf->cell(10, 10, $idNew->solici_de_compra, 0, 0, 'R', 0);
            $pdf->Line(135, 36, 151, 36);
        } elseif ($solici_de_compraTamano == 4) {
            $pdf->SetX(-73);
            $pdf->cell(10, 10, $idNew->solici_de_compra, 0, 0, 'R', 0);
            $pdf->Line(135, 36, 151, 36);
        } elseif ($solici_de_compraTamano == 5) {
            $pdf->SetX(-72);
            $pdf->cell(10, 10, $idNew->solici_de_compra, 0, 0, 'R', 0);
            $pdf->Line(135, 36, 151, 36);
        } elseif ($solici_de_compraTamano == 6) {
            $pdf->SetX(-71);
            $pdf->cell(10, 10, $idNew->solici_de_compra, 0, 0, 'R', 0);
            $pdf->Line(135, 36, 151, 36);
        } elseif ($solici_de_compraTamano == 7 || $solici_de_compraTamano == 8) {
            $pdf->SetX(-70);
            $pdf->cell(10, 10, $idNew->solici_de_compra, 0, 0, 'R', 0);
            $pdf->Line(135, 36, 151, 36);
        } elseif ($solici_de_compraTamano == 9) {
            $pdf->SetX(-69);
            $pdf->cell(10, 10, $idNew->solici_de_compra, 0, 0, 'R', 0);
            $pdf->Line(135, 36, 151, 36);
        } elseif ($solici_de_compraTamano = 10) {
            $pdf->SetX(-68);
            $pdf->cell(10, 10, $idNew->solici_de_compra, 0, 0, 'R', 0);
            $pdf->Line(135, 36, 151, 36);
        }
        ...
        $pdf->SetY(36);
        $pdf->SetX(-106);
        $pdf->Cell(10, 10, 'ACREDITACION: ');
        $pdf->SetFont('Arial', 'B', 8);
        $pdf->SetX(-75);
        $pdf->cell(10, 10, $idNew->acreditacion, 0, 0, 'R', 0);
        $pdf->Line(135, 43, 151, 43);
        $pdf->SetFont('Arial', '', 8);
        $pdf->SetY(43);
        $pdf->SetX(-104);
        $pdf->Cell(10, 10, 'COD DE SERV: ');
        $pdf->SetFont('Arial', 'B', 8);
        $pdf->SetX(-75);
        $pdf->cell(10, 10, $idNew->acreditacion, 0, 0, 'R', 0);
        $pdf->Line(135, 50, 151, 50);

        $pdf->SetFont('Arial', '', 8);
        $pdf->SetY(10);
        $pdf->SetX(-45);
        $pdf->Cell(10, 20, utf8_decode('FECHA ORD: '));
        $pdf->SetFont('Arial', 'B', 8);
        $pdf->SetX(-18);
        $pdf->cell(10, 20, $idNew->fecha_ordinario, 0, 0, 'R', 0);
        $pdf->SetFont('Arial', '', 8);
        $pdf->SetY(17);
        $pdf->SetX(-45);
        $pdf->Cell(10, 20, utf8_decode('FECHA REF: '));
        $pdf->SetFont('Arial', 'B', 8);
        $pdf->SetX(-18);
        ...
        $pdf->SetY(24);
        $pdf->SetX(-45);
        $pdf->Cell(10, 20, utf8_decode('FECHA SC: '));
        $pdf->SetFont('Arial', 'B', 8);
        $pdf->SetX(-18);
        $pdf->cell(10, 20, $idNew->fecha_referencia, 0, 0, 'R', 0);
        $pdf->SetFont('Arial', '', 8);
        ...
        $pdf->SetY(38);
        $pdf->SetX(-45);
        $pdf->Cell(10, 20, utf8_decode('EXISTENCIA: '));
        $pdf->SetFont('Arial', 'B', 8);
        $pdf->SetX(-18);
        $pdf->cell(10, 20, $idNew->exigencia, 0, 0, 'R', 0);


        $pdf->SetY(60);
        $pdf->SetFont('Arial', '', 8);
        $pdf->Write(5, utf8_decode('JSLL/JAR/sfc'));
        $pdf->Ln();
        $pdf->Write(5, utf8_decode('DE: JEFE DE SUB DEPARTAMENTO MANTENIMIENTO'));
        $pdf->Ln();
        $pdf->Write(5, utf8_decode('A: SR. SUBDIRECTOR ADMINISTRATIVO - HOSPITAL BASE VALDIVIA'));
        $pdf->Ln();
        $pdf->Ln();
        $pdf->SetFont('Arial', '', 9);
        $pdf->Write(5, utf8_decode('MATERIA: Solicita aprobar documentación adjunta:'));
        $pdf->Ln();
        $pdf->Write(5, utf8_decode('       A)  '));      // 2  Tab - 1 Tab
        $pdf->SetFont('Arial', 'B', 8);
        $pdf->Write(5, $idNew->doc_adjunta);
        $pdf->SetFont('Arial', '', 9);
        $pdf->Write(5, utf8_decode(' para '));
        $pdf->SetFont('Arial', 'B', 8);
        $pdf->Write(5, $idNew->tipo_compra);
        $pdf->SetFont('Arial', '', 9);
        $pdf->Write(5, utf8_decode(', en la calidad de tramitación '));
        $pdf->SetFont('Arial', 'B', 8);
        $pdf->Write(5, $idNew->exigencia);
        $pdf->Ln();
        $pdf->Ln();
        $pdf->Ln();
        $pdf->SetFont('Arial', '', 9);
        $pdf->Write(5, utf8_decode('Agradeceré a usted autorice la: COMPRA; REPARACIÓN; Y/O CONTRATACIÓN DE SERVICIOS, de:'));
        $pdf->Ln();
        $pdf->SetFont('Arial', 'B', 8);
        ...
        $pdf->Cell(30, 5, 'DETALLE', 1, 0);
        $pdf->Cell(20, 5, 'EQUIPO', 1, 0);
        $pdf->Cell(20, 5, 'MARCA', 1, 0);
        $pdf->Cell(17, 5, 'MODELO', 1, 0);
        $pdf->Cell(15, 5, 'SERIE', 1, 0);
        $pdf->Cell(15, 5, 'N INV', 1, 0);
        $pdf->Cell(19, 5, 'COD MANG', 1, 0);
        $pdf->Cell(17, 5, 'V UNI', 1, 0);
        $pdf->Cell(17, 5, 'V NETO', 1, 0);
