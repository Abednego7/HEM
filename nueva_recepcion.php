...
if (!isset($_SESSION['nombre'])) {
    ...
    if (
        $_SESSION['tipo'] === "ADMINISTRADOR" || $_SESSION['tipo'] === "PROYECTOS"
    ) {
...
<body>
    ...
    <div class="container-fluid">
        <div class="container-fluid">
            <div class="row">
                <h2 style="text-align:center; margin-top: 20px;">NUEVA RECEPCIÓN EQUIPO</h2>
            </div><br><br>
                                    ...
                                    <tr>
                                        <td>
                                            ...
                                            if (empty($idsRecepcion)) {                                      // VALIDA SI relacion_id ESTA VACIA
                                                ...
                                                echo '<a href="vistas/ingreso/ingreso_recepcion.php?id_relacion_key=' . $dato->id_relacion . '"><i class="fas fa-concierge-bell fa-2x" style="color: mediumseagreen;"></i></i></a>';
                                            }
    ...
    <!-- SweetAlert2 Copy Save -->
    <script>
        const cTest = $('.copyTest').data('ctest') // data-ctest    "ctest"
        if (cTest) {
            Swal.fire({
                icon: 'error',
                title: 'Registro <?php echo $_GET["result_copy"] ?>, ya cuenta con una recepción',
                showConfirmButton: true,
                timer: 5500
            }).then((result) => {
                if (result.isConfirmed) {
                    if (result.value) {
                        window.location = 'nueva_recepcion.php';
                    }
                }
            })
        }
